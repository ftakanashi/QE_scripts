...
from transformers.modeling_bert import BertPreTrainedModel, BertModel, BERT_INPUTS_DOCSTRING, _TOKENIZER_FOR_DOC, \
    _CONFIG_FOR_DOC
...
class PsdalignSquadProcessor(SquadV2Processor):
    ...
    # override the original _create_example method to add qe_tag information
...
class PsdalignSquadResult:
    ...
    def __init__(self, unique_id, start_logits, end_logits, qe_tag_logits, start_top_index=None, end_top_index=None,
                 cls_logits=None):
        self.start_logits = start_logits
        self.end_logits = end_logits
        self.qe_tag_logits = qe_tag_logits
...
@dataclass
class QETagPredictionModelOutput(ModelOutput):
    loss: Optional[torch.FloatTensor] = None
    start_logits: torch.FloatTensor = None
    end_logits: torch.FloatTensor = None
    qe_tag_logits: torch.FloatTensor = None
    hidden_states: Optional[Tuple[torch.FloatTensor]] = None
    attentions: Optional[Tuple[torch.FloatTensor]] = None


class BertForQETagPrediction(BertPreTrainedModel):
    ...
    #     output_type=QETagPredictionModelOutput,
    ...
    def forward(
        ...
        qe_tag_logits = self.qe_tag_outputs(cls_output)
        if qe_tags is not None:
            qe_tag_loss = CrossEntropyLoss()(qe_tag_logits, qe_tags)
            total_loss += qe_tag_loss

        if not return_dict:
            output = (start_logits, end_logits, qe_tag_logits) + outputs[2:]
            return ((total_loss,) + output) if total_loss is not None else output

        return QETagPredictionModelOutput(
            loss=total_loss,
            start_logits=start_logits,
            end_logits=end_logits,
            qe_tag_logits=qe_tag_logits,
...
def compute_predictions_logits(
    ...
    _PrelimPrediction = collections.namedtuple(  # pylint: disable=invalid-name
        "PrelimPrediction", ["feature_index", "start_index", "end_index", "start_logit", "end_logit", "qe_tag"]
    )
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        class _NbestPrediction(object):
            def __init__(self, text, start_logit, end_logit, start_index, end_index, probability, qe_tag):
                self.text = text
        ...
        if version_2_with_negative:
            if "" not in seen_predictions:
                nbest.append(_NbestPrediction(text="", start_logit=null_start_logit, end_logit=null_end_logit,
                                              start_index=-1, end_index=-1, probability=0.0, qe_tag=0))
...
def squad_convert_examples_to_features(
    ...
    if return_dataset == "pt":
        ...
        all_qe_tag = torch.tensor([f.qe_tag == 'OK' for f in features], dtype=torch.long)
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        for i, feature_index in enumerate(feature_indices):
            ...
            if len(output) >= 5:
                ...
                start_logits, end_logits, qe_tag_logits = output
                result = PsdalignSquadResult(unique_id, start_logits, end_logits, qe_tag_logits)
...
def main():
    ...
    model = BertForQETagPrediction.from_pretrained(
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model = BertForQETagPrediction.from_pretrained(checkpoint)
            model.to(args.device)
