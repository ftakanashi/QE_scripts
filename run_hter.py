...
'''
    ...
    --source_qe_tags train.source_tag --mt_qe_tags train.tag --max_seq_length 384 --per_device_train_batch_size 8
    --learning_rate 3e-5 --num_train_epochs 3.0 --output_dir fine-tuned
...
class HTERScoreDataset(Dataset):
    ...
    def __init__(self, args, tokenizer, set_type):
        ...
        for i, e in enumerate(examples):
            ...
            for k, token in enumerate(tokenizer.convert_ids_to_tokens(batch_text_encoding.input_ids[i])):
                if token == tokenizer.pad_token:
                    break
                if token.find('##') != -1:  # tokenized by wordpiece
                    qe_tag_encoding.insert(k, qe_tag_encoding[k - 1])
