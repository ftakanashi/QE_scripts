...
class HTERScoreDataProcessor(DataProcessor):
    ...
    def get_examples(self, set_type):
        ...
        for src_line, mt_line, src_qe_tags_line, mt_qe_tags_line, hter_scores_line in \
            ...
            guid = f'{set_type}-{i}'
            examples.append(
                HTERScoreInputExample(guid=guid, source_text=src_line, mt_text=mt_line, source_qe_tags=src_qe_tags_line,
...
class HTERScoreDataset(Dataset):
    ...
    def __init__(self, args, tokenizer, set_type):
        ...
        for i, e in enumerate(examples):
            ...
            qe_tag_encoding = ['OK'] + e.source_qe_tags.split() + ['OK'] + e.mt_qe_tags.split()[1::2] + ['OK']

            origin_tokens = f'{tokenizer.cls_token} {e.source_text} {tokenizer.sep_token} {e.mt_text} ' \
                            f'{tokenizer.sep_token}'.split()
            pieced_tokens = tokenizer.convert_ids_to_tokens(batch_text_encoding.input_ids[i])
            pieced_to_origin_mapping = self.map_offset(origin_tokens, pieced_tokens)
    ...
    def map_offset(self, orig_tokens, pieced_tokens,
                   no_normalize_tokens=['[CLS]', '[SEP]'], ignore_tokens=['[PAD]']):
        '''
        ...
        NOT strictly tested, looks good when working with German.
        '''
        mapping = {}
        pieced_i = 0

        cleaned_pieced_token = [t for t in pieced_tokens if t not in ignore_tokens]
        pieced_tokens = cleaned_pieced_token

        for orig_i, orig_token in enumerate(orig_tokens):
            tmp_token = pieced_tokens[pieced_i]

            if orig_token not in no_normalize_tokens:
